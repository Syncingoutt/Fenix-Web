...
export const ZONE_NAMES: Record<string, string> = {
  ...
  "KD_AiRenDiSanCeng": "Blistering Lava Sea - Heart of Mountains",
  ...
  "GeBuLinCunLuo": "Glacial Abyss - Demiman Village",
  ...
  "YL_BeiFengLinDi": "Glacial Abyss - Grimwind Woods",
...
export const AMBIGUOUS_ZONES: Record<string, Record<number, string>> = {
  ...
  "SD_GeBuLinCunLuo": {  // Demiman Village / Grove of Calamity
    36: "Glacial Abyss - Demiman Village",
  },
};

// Exact LevelId mappings for special zones (bosses, secret realms, etc.)
export const LEVEL_ID_ZONES: Record<number, string> = {
 ...
 *
 * @param zonePath - Internal zone path like /Game/Art/Maps/01SD/XZ_YuJinZhiXiBiNanSuo200/...
 * @param levelId - Optional LevelId for differentiating zones with same path
 * @returns English display name if known, otherwise a cleaned-up version of the path
 */
export function getZoneDisplayName(zonePath: string, levelId?: number): string {
  // First, check exact level_id mapping (for boss zones, secret realms, etc.)
  if (levelId !== undefined && LEVEL_ID_ZONES[levelId]) {
    return LEVEL_ID_ZONES[levelId];
  }

  // Check if this is an ambiguous zone that needs suffix-based resolution
  if (levelId !== undefined) {
    for (const [zonePattern, suffixMap] of Object.entries(AMBIGUOUS_ZONES)) {
      if (zonePath.includes(zonePattern)) {
        const suffix = levelId % 100;
        if (suffixMap[suffix]) {
  ...
  for (const [internalName, englishName] of Object.entries(ZONE_NAMES)) {
    if (zonePath.includes(internalName)) {
      return englishName;
    }
  }
  ...
  for (let i = parts.length - 1; i >= 0; i--) {
    const part = parts[i];
    if (part && !part.startsWith("Game") && !part.startsWith("Art")) {
